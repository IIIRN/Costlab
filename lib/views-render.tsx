...
export async function renderViewForId(id: string, query?: Record<string, string | string[] | undefined>) {
  ...
  const form = view.table && usesSchemaForm(view.id)
    ? await getFormPayload(view.table).catch(() => null)
    : null;

  return await renderView(view, search, query, displayName, form);
}
...
async function renderView(
  ...
  if (view.type === "table" && view.table) {
    ...
    const [rawRows, headers, form, projectDataRows, companyRows, bankRows] = await Promise.all([
      safeRows(view.table),
      getHeaders(view.table).catch(() => []),
      initialForm ? Promise.resolve(initialForm) : (usesSchemaForm(view.id) ? getFormPayload(view.table).catch(() => null) : Promise.resolve(null)),
