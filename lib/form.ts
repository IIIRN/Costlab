...
async function getFormSchemaWithSheetOptions(tableName: string): Promise<FieldSchema[]> {
  ...
  return schema.map(field => {
    ...
    // Inject dynamic options from ตัวเลือกระบบ sheet if column matching field name exists
    if (systemOptions[field.name] && systemOptions[field.name].length > 0) {
      fieldValues = systemOptions[field.name];
    }
