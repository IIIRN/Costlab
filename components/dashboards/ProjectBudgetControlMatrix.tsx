  ...
  return (
    <div className="bg-white text-slate-900 rounded-lg border border-slate-200 p-4 sm:p-5 space-y-4 font-sans my-4 max-w-full">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5">
            <select
              ...
              className="bg-white border border-slate-300 text-xs font-bold text-slate-900 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-400 min-w-[200px]"
            >
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2 bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3 shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
                ...
                <span className="text-xs text-slate-500 font-semibold">Work Group Analysis</span>
              </div>
                    ...
                    <div key={wg.groupTitle} className="bg-white border border-slate-200 rounded-md p-3.5 space-y-2 shadow-xs">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs">
                        <span className="font-bold text-slate-900 flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-slate-500 font-semibold">({wg.itemsCount} หมวดย่อย)</span>
                        </span>

                        <div className="flex items-center gap-2 text-xs font-bold">
                          <span className="text-slate-700">Cap: <strong className="text-slate-900">{money(wg.capSum)}</strong> ฿</span>
                          <span className="text-slate-300">|</span>
                          <span className="text-slate-700">Spent: <strong className="text-emerald-800">{money(wg.spentSum)}</strong> ฿</span>
                          {wg.isOver ? (
                            <span className="px-2 py-0.5 bg-rose-100 text-rose-800 rounded-full font-bold text-[11px] border border-rose-300">เกินงบ</span>
                          ) : wg.isWarning ? (
                            <span className="px-2 py-0.5 bg-amber-100 text-amber-900 rounded-full font-bold text-[11px] border border-amber-300">เฝ้าระวัง</span>
                          ) : (
                            <span className="px-2 py-0.5 bg-emerald-100 text-emerald-900 rounded-full font-bold text-[11px] border border-emerald-300">ปกติ</span>
                          )}
                        </div>
                      </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3 shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
                ...
                <span className="text-xs text-slate-500 font-semibold">Top 5 Spending</span>
              </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3 shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
              ...
              <span className="text-xs text-slate-500 font-semibold">Group Breakdown</span>
            </div>
      ...
      {(viewMode === "all" || viewMode === "table") && (
        <div className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm space-y-3 p-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 bg-white border border-slate-300 rounded-md text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-400 w-full sm:w-60"
              />
            </div>
          </div>
          ...
          <div className="overflow-x-auto max-h-[550px] border border-slate-200 rounded-md">
            <table className="w-full text-left text-xs border-collapse font-sans">
              <thead className="bg-slate-900 text-white font-bold sticky top-0 z-10">
                <tr>
                  <th className="py-3 px-3.5 font-bold">หมวดสินค้า / ประเภทงาน</th>
                  <th className="py-3 px-3.5 text-right font-bold">วงเงินคุมงบ (Cap)</th>
                  <th className="py-3 px-3.5 text-right text-emerald-300 font-bold">จ่ายจริง (Actual)</th>
                  <th className="py-3 px-3.5 text-right font-bold">งบคงเหลือ / เกินงบ</th>
                  <th className="py-3 px-3.5 text-center w-40 font-bold">สัดส่วนการใช้วงเงิน</th>
                  <th className="py-3 px-3.5 text-center font-bold">สถานะ</th>
                  <th className="py-3 px-3.5 text-center font-bold">ดูรายการบิล</th>
                </tr>
              </thead>
                    ...
                    <Fragment key={groupTitle}>
                      ...
                      {groupItems.map((cat, idx) => (
                        <tr key={`${cat.field}-${idx}`} className="hover:bg-slate-50 transition border-b border-slate-100">
                          {/* Category Label */}
                          <td className="py-2.5 px-3.5 font-bold text-slate-900 flex items-center gap-2 pl-6">
                          ...
                          <td className="py-2.5 px-3.5 text-center">
                            {cat.budgetCap === 0 ? (
                              <span className="text-xs font-bold text-slate-700 bg-slate-100 border border-slate-300 px-2.5 py-0.5 rounded-full">ทั่วไป</span>
                            ) : cat.isOver ? (
                              <span className="text-xs font-bold bg-rose-100 text-rose-900 border border-rose-300 px-2.5 py-0.5 rounded-full inline-flex items-center justify-center gap-1">
                                <AlertCircle size={13} /> เกินงบ
                              </span>
                            ) : cat.isWarning ? (
                              <span className="text-xs font-bold bg-amber-100 text-amber-950 border border-amber-300 px-2.5 py-0.5 rounded-full inline-flex items-center justify-center gap-1">
                                <AlertTriangle size={13} /> เฝ้าระวัง
                              </span>
                            ) : (
                              <span className="text-xs font-bold bg-emerald-100 text-emerald-950 border border-emerald-300 px-2.5 py-0.5 rounded-full inline-flex items-center justify-center gap-1">
                          ...
                          <td className="py-2.5 px-3.5 text-center">
                            {cat.matchingBills.length > 0 ? (
                              <button
                                ...
                                className="px-3 py-1 bg-slate-100 hover:bg-slate-200 text-slate-900 text-xs font-bold rounded-md border border-slate-300 transition flex items-center justify-center gap-1 mx-auto cursor-pointer shadow-xs"
                              >
                                <span>{cat.matchingBills.length} บิล</span>
          ...
          <div className="bg-white text-slate-900 rounded-xl max-w-4xl w-full max-h-[85vh] flex flex-col border border-slate-300 shadow-2xl overflow-hidden animate-in fade-in duration-150">
            <div className="px-5 py-3.5 border-b border-slate-200 bg-slate-100 flex items-center justify-between">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-700 font-bold p-1 rounded-md hover:bg-slate-200 transition cursor-pointer"
              >
                <X size={18} />
              </button>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse font-sans">
                <thead className="bg-slate-900 text-white font-bold sticky top-0">
                  <tr>
                    ...
                    <th className="py-2.5 px-3">ร้านค้า / รายการ</th>
