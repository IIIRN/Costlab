...
export function DualSidebar({ collapsed, onToggleCollapse, currentUser }: DualSidebarProps) {
  ...
  const loadCompanySettings = async () => {
    try {
      ...
      if (cached) {
        ...
        if (parsed.logoUrl && (parsed.logoUrl.startsWith("data:") || parsed.logoUrl.length > 500)) {
          parsed.logoUrl = "";
          localStorage.setItem("costlab_company_settings", JSON.stringify(parsed));
        }
      ...
      if (json.success && json.settings) {
        if (json.settings.logoUrl && (json.settings.logoUrl.startsWith("data:") || json.settings.logoUrl.length > 500)) {
  ...
  return (
    <aside className={`hidden md:flex fixed top-0 bottom-0 left-0 h-screen select-none bg-[#0b3531] text-slate-100 border-r border-[#062e2b] z-30 font-sans font-normal transition-all duration-300 ${collapsed ? "w-[60px]" : "w-[260px]"}`}>
      ...
      <div className="w-[60px] flex-shrink-0 h-full border-r border-[#0d3f3a] flex flex-col items-center justify-between py-3.5 bg-[#062e2b]">
        ...
        <div className="flex flex-col items-center gap-3.5">
          ...
          >
            {companySettings.logoUrl ? (
              ...
              <div className="flex gap-1 items-center justify-center font-bold text-[#d4f54e] text-sm">
                CL
              </div>
            )}
          </Link>
            ...
            <button
              ...
              className={`w-9 h-9 rounded-md flex items-center justify-center transition-all ${
                currentTab === "main" || (currentTab === "all" && !isSettingsActive && !pathname.startsWith("/views/"))
                  ? "bg-[#d4f54e] text-[#0b3531] font-medium"
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <button
            ...
            className="w-8 h-8 rounded-md border border-[#144d47] bg-[#0b3531] text-[#9eb5b0] hover:text-white hover:bg-[#13443e] flex items-center justify-center transition cursor-pointer"
      ...
      {/* 2. RIGHT SECONDARY DRAWER PANEL (200px) */}
      {!collapsed && (
        <div className="w-[200px] flex-shrink-0 flex flex-col h-full bg-[#0b3531] text-slate-100 overflow-hidden border-r border-[#062e2b] font-sans font-normal">
          ...
          <div className="p-3 border-b border-[#12443e] bg-[#0b3531] sticky top-0 z-10 space-y-2">
            ...
            <div className="px-0.5 py-0.5">
              <div className="text-xs font-semibold text-white leading-snug break-words font-sans">
                {companySettings.companyName || "CostLab Executive"}
              </div>
              {companySettings.companySubTitle && (
                <div className="text-xs text-[#8eaba5] font-normal leading-tight mt-0.5 truncate font-sans">
                  {companySettings.companySubTitle}
                </div>
              )}
            </div>
            ...
            <div className="relative flex items-center">
              <Search size={14} className="absolute left-2.5 text-[#7e9f9a] pointer-events-none" />
              <input
                type="text"
                ...
                autoComplete="off"
                className="w-full bg-[#062e2b] text-white text-xs pl-8 pr-2.5 py-1.5 rounded-md border border-[#164e48] focus:border-[#d4f54e] focus:outline-none placeholder:text-[#6e8e88] font-sans font-normal transition"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2.5 space-y-4 text-xs font-sans font-normal">
            ...
            {(currentTab === "all" || currentTab === "main") && (
              <div className="space-y-1.5">
                <div className="px-1.5 pb-1 flex items-center justify-between text-xs font-semibold text-[#7fa39d] uppercase font-sans">
                      ...
                      >
                        <span
                          className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 transition-colors ${
                            active ? "bg-[#0b3531] text-[#d4f54e]" : "bg-[#072825] text-[#8ea8a3]"
                          }`}
                        >
                          <Icon size={14} />
                        </span>
            ...
            {(currentTab === "all" || currentTab === "system") && (
              <div className="space-y-1.5">
                <div className="px-1.5 pb-1 text-xs font-semibold text-[#7fa39d] uppercase font-sans">
                ...
                <div className="space-y-1">
                  ...
                  >
                    <span
                      className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 transition-colors ${
                        isGeneralSettingsActive ? "bg-[#0b3531] text-[#d4f54e]" : "bg-[#072825] text-[#8ea8a3]"
                      }`}
                    >
                      <Building2 size={14} />
                    </span>
                  ...
                  >
                    <span
                      className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 transition-colors ${
                        pathname === "/settings" ? "bg-[#0b3531] text-[#d4f54e]" : "bg-[#072825] text-[#8ea8a3]"
                      }`}
                    >
                      <Database size={14} />
                    </span>
                  ...
                  >
                    <span
                      className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 transition-colors ${
                        pathname.startsWith("/settings/product-categories") ? "bg-[#0b3531] text-[#d4f54e]" : "bg-[#072825] text-[#8ea8a3]"
                      }`}
                    >
                      <Package size={14} />
                    </span>
                  ...
                  >
                    <span
                      className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 transition-colors ${
                        pathname.startsWith("/settings/options") ? "bg-[#0b3531] text-[#d4f54e]" : "bg-[#072825] text-[#8ea8a3]"
                      }`}
                    >
                      <Sliders size={14} />
                    </span>
                  ...
                  >
                    <span
                      className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 transition-colors ${
                        pathname.startsWith("/settings/line-system") || pathname.startsWith("/line-system") ? "bg-[#0b3531] text-[#d4f54e]" : "bg-[#072825] text-[#8ea8a3]"
                      }`}
                    >
                  ...
                  >
                    <span
                      className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 transition-colors ${
                        pathname.startsWith("/settings/users") || pathname.startsWith("/users") ? "bg-[#0b3531] text-[#d4f54e]" : "bg-[#072825] text-[#8ea8a3]"
                      }`}
                    >
                      <IdCard size={14} />
                    </span>
